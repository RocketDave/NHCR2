create or replace function get_findings_diagnostic(
...
begin
    ...
    select count(*) into lcl_total_you from Findings where diagnostic = 1 and 
            endoscopist_id = lcl_endoscopist_id and facility_id = lcl_facility_id and 
            exam_date <= in_end_dt and completed = 1;
    select count(*) into lcl_total_site from Findings where diagnostic = 1 and 
            facility_id = lcl_facility_id and 
            exam_date <= in_end_dt and completed = 1;
    select count(*) into lcl_total_nhcr from Findings where diagnostic = 1 and 
            exam_date >= in_start_dt_nhcr and exam_date <= in_end_dt2 and completed = 1;

    select count(*) into lcl_count_you from Findings  where diagnostic = 1 and 
            endoscopist_id = lcl_endoscopist_id and facility_id = lcl_facility_id and 
            exam_date <= in_end_dt and completed = 1 and find_calc_normal = 1;
    select count(*) into lcl_count_site from Findings where diagnostic = 1 and 
            facility_id = lcl_facility_id and 
            exam_date <= in_end_dt and completed = 1 and find_calc_normal = 1;
    select count(*) into lcl_count_nhcr from Findings  where diagnostic = 1 and 
    ...
    return query select t.finding,t.you,t.site,t.nhcr from lcl_table t;
end;
$BODY$
LANGUAGE plpgsql 
