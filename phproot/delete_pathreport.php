...
if ($path_report_id != "") {
    $result = pg_query("select * from delete_path_report (".$path_report_id.')');
    if ($result) {
...
<?php
    if ($submit_message != "Record Deleted") {
        echo '<div class="form-group row">
            <div class="text-center">
                <button id="to_pathreport" type="button" class="btn btn-link">Return to Path Report</button>
            </div>
        </div>';
    }
?>
        <div class="text-center">
            <button id="to_pathreports" type="button" class="btn btn-link">Return to Path Reports</button>
        </div>
...
<script>
    $("#to_pathreport").on('click', function () {
            $("#myform").attr("action","pathreport.php");
            $('#myform').submit();
    });
    $("#to_pathreports").on('click', function () {
            $("#myform").attr("action","pathreports.php");
            $('#myform').submit();
    });
</script>
</body>
