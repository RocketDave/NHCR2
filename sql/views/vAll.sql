CREATE OR REPLACE VIEW vAll AS 
         SELECT p.c_event_id AS event_id, c.c_event_id, p.p2_event_id, 
            ...
            p.sas_key_id, p.discrepnote, c.facility_id, c.facility_type, 
            ...
            c.find_calc_nodata
           FROM vpolypspecimen p
      left join vcolo c ON c.c_event_id = p.c_event_id 
     WHERE p.c_event_id IS NOT NULL AND p.s_event_id IS NULL -- polyp2 and colo only , no specimen
UNION 
         SELECT p.s_event_id AS event_id, c.c_event_id,  p.p2_event_id, 
            p.s_event_id, p.old_polyp_id, p.p_loc, p.p_siz, p.pt_cb, p.pt_hb, 
            p.pt_hs, p.pt_cs, p.pt_pme, p.pt_pe, p.pt_nr, p.pt_lo, p.pt_o, 
            p.pt_sn, p.pt_te, p.p2_notes, p.p_flat, p.specimen_id, 
            p.path_report_id, p.path_polyp_loc, p.polyp_num, 
            ...
            c.find_calc_nodata
           FROM vpolypspecimen p
      left JOIN vcolo c ON c.c_event_id = p.s_event_id -- all with a specimen record
     WHERE p.s_event_id IS NOT NULL;
