create or replace function set_specimen(
in in_specimen_id integer,
in in_path_report_id integer,
in in_path_polyp_loc character varying,
in in_polyp_num character varying,
in in_discrepnote character varying,
in in_container character varying,
in in_fragment character varying,
in in_other_dx_specify character varying,
in in_site_location_cm integer,
in in_site_desc character varying,
in in_size_mm integer,
in in_flg_no_path_spec integer,
in in_ptype_carcinoid integer,
in in_ptype_ganglio integer,
in in_ptype_hamart integer,
in in_ptype_hp integer,
in in_ptype_inflam integer,
in in_ptype_juvenile integer,
in in_ptype_lelomyoma integer,
in in_ptype_lipoma integer,
in in_ptype_mp integer,
in in_ptype_norm_muc integer,
in in_ptype_not_polyp integer,
in in_ptype_other integer,
in in_ptype_pautzjeg integer,
in in_ptype_sa integer,
in in_ptype_ssp integer,
in in_ptype_mixed integer,
in in_ptype_ta integer,
in in_ptype_tva integer,
in in_ptype_va integer,
in in_flg_dx integer,
in in_flg_no_discrep integer,
in in_flg_no_q_spec integer,
in in_flg_num_polyps integer,
in in_flg_site_discrep integer,
in in_flg_site_uncert integer,
in in_flg_path_sites integer,
in in_flg_review integer,
in in_hgd integer,
in in_ibd_ibd integer,
in in_ibd_actcol integer,
in in_ibd_chroncol integer,
in in_ibd_coloth integer,
in in_ibd_inactcol integer,
in in_ibd_lgdysp integer,
in in_n_intra_ca integer,
in in_n_inv_ca integer,
in in_n_cancer integer,
in in_ptype_fibroblast integer,
in in_ptype_lymphoid integer,
in in_flg_assump integer,
in in_flg_multis integer,
in in_flg_multisites integer,
in in_flg_residual integer,
in in_flg_assump_numpolyps integer,
in in_flg_dx_size integer,
in in_flg_dx_site integer,
in in_flg_dx_multis integer,
in in_specimen_type character varying,
in in_notes character varying,
in in_t_class character varying,
in in_n_class character varying,
in in_y_prefix integer,
in in_record_complete integer,
in in_flg_size_discrep integer)

returns table (
    lcl_specimen_id integer, 
    lcl_message character varying) AS
$BODY$

begin

    if (in_specimen_id = -9) then
        select 1 + max(specimen_id) from specimen into in_specimen_id;
    end if;

    if exists(select * from specimen where specimen_id =  in_specimen_id) then
        update specimen set 
            path_report_id = in_path_report_id,
            path_polyp_loc = in_path_polyp_loc,
            polyp_num = in_polyp_num,
            discrepnote = in_discrepnote,
            container = in_container,
            fragment = in_fragment,
            other_dx_specify = in_other_dx_specify,
            site_location_cm = in_site_location_cm,
            site_desc = in_site_desc,
            size_mm = in_size_mm,
            flg_no_path_spec = in_flg_no_path_spec,
            ptype_carcinoid = in_ptype_carcinoid,
            ptype_ganglio = in_ptype_ganglio,
            ptype_hamart = in_ptype_hamart,
            ptype_hp = in_ptype_hp,
            ptype_inflam = in_ptype_inflam,
            ptype_juvenile = in_ptype_juvenile,
            ptype_lelomyoma = in_ptype_lelomyoma,
            ptype_lipoma = in_ptype_lipoma,
            ptype_mp = in_ptype_mp,
            ptype_norm_muc = in_ptype_norm_muc,
            ptype_not_polyp = in_ptype_not_polyp,
            ptype_other = in_ptype_other,
            ptype_pautzjeg = in_ptype_pautzjeg,
            ptype_sa = in_ptype_sa,
            ptype_ssp = in_ptype_ssp,
            ptype_mixed = in_ptype_mixed,
            ptype_ta = in_ptype_ta,
            ptype_tva = in_ptype_tva,
            ptype_va = in_ptype_va,
            flg_dx = in_flg_dx,
            flg_no_discrep = in_flg_no_discrep,
            flg_no_q_spec = in_flg_no_q_spec,
            flg_num_polyps = in_flg_num_polyps,
            flg_site_discrep = in_flg_site_discrep,
            flg_site_uncert = in_flg_site_uncert,
            flg_path_sites = in_flg_path_sites,
            flg_review = in_flg_review,
            hgd = in_hgd,
            ibd_ibd = in_ibd_ibd,
            ibd_actcol = in_ibd_actcol,
            ibd_chroncol = in_ibd_chroncol,
            ibd_coloth = in_ibd_coloth,
            ibd_inactcol = in_ibd_inactcol,
            ibd_lgdysp = in_ibd_lgdysp,
            n_intra_ca = in_n_intra_ca,
            n_inv_ca = in_n_inv_ca,
            n_cancer = in_n_cancer,
            ptype_fibroblast = in_ptype_fibroblast,
            ptype_lymphoid = in_ptype_lymphoid,
            flg_assump = in_flg_assump,
            flg_multis = in_flg_multis,
            flg_multisites = in_flg_multisites,
            flg_residual = in_flg_residual,
            flg_assump_numpolyps = in_flg_assump_numpolyps,
            flg_dx_size = in_flg_dx_size,
            flg_dx_site = in_flg_dx_site,
            flg_dx_multis = in_flg_dx_multis,
            specimen_type = in_specimen_type,
            notes = in_notes,
            t_class = in_t_class,
            n_class = in_n_class,
            y_prefix = in_y_prefix,
            record_complete = in_record_complete,
            flg_size_discrep = in_flg_size_discrep
            where specimen_id = cast (in_specimen_id as integer);
        else insert into specimen (
            specimen_id,
            path_report_id,
            path_polyp_loc,
            polyp_num,
            discrepnote,
            container,
            fragment,
            other_dx_specify,
            site_location_cm,
            site_desc,
            size_mm,
            flg_no_path_spec,
            ptype_carcinoid,
            ptype_ganglio,
            ptype_hamart,
            ptype_hp,
            ptype_inflam,
            ptype_juvenile,
            ptype_lelomyoma,
            ptype_lipoma,
            ptype_mp,
            ptype_norm_muc,
            ptype_not_polyp,
            ptype_other,
            ptype_pautzjeg,
            ptype_sa,
            ptype_ssp,
            ptype_mixed,
            ptype_ta,
            ptype_tva,
            ptype_va,
            flg_dx,
            flg_no_discrep,
            flg_no_q_spec,
            flg_num_polyps,
            flg_site_discrep,
            flg_site_uncert,
            flg_path_sites,
            flg_review,
            hgd,
            ibd_ibd,
            ibd_actcol,
            ibd_chroncol,
            ibd_coloth,
            ibd_inactcol,
            ibd_lgdysp,
            n_intra_ca,
            n_inv_ca,
            n_cancer,
            ptype_fibroblast,
            ptype_lymphoid,
            flg_assump,
            flg_multis,
            flg_multisites,
            flg_residual,
            flg_assump_numpolyps,
            flg_dx_size,
            flg_dx_site,
            flg_dx_multis,
            specimen_type,
            notes,
            t_class,
            n_class,
            y_prefix,
            record_complete,
            flg_size_discrep
        )
        values (
            in_specimen_id,
            in_path_report_id,
            in_path_polyp_loc,
            in_polyp_num,
            in_discrepnote,
            in_container,
            in_fragment,
            in_other_dx_specify,
            in_site_location_cm,
            in_site_desc,
            in_size_mm,
            in_flg_no_path_spec,
            in_ptype_carcinoid,
            in_ptype_ganglio,
            in_ptype_hamart,
            in_ptype_hp,
            in_ptype_inflam,
            in_ptype_juvenile,
            in_ptype_lelomyoma,
            in_ptype_lipoma,
            in_ptype_mp,
            in_ptype_norm_muc,
            in_ptype_not_polyp,
            in_ptype_other,
            in_ptype_pautzjeg,
            in_ptype_sa,
            in_ptype_ssp,
            in_ptype_mixed,
            in_ptype_ta,
            in_ptype_tva,
            in_ptype_va,
            in_flg_dx,
            in_flg_no_discrep,
            in_flg_no_q_spec,
            in_flg_num_polyps,
            in_flg_site_discrep,
            in_flg_site_uncert,
            in_flg_path_sites,
            in_flg_review,
            in_hgd,
            in_ibd_ibd,
            in_ibd_actcol,
            in_ibd_chroncol,
            in_ibd_coloth,
            in_ibd_inactcol,
            in_ibd_lgdysp,
            in_n_intra_ca,
            in_n_inv_ca,
            in_n_cancer,
            in_ptype_fibroblast,
            in_ptype_lymphoid,
            in_flg_assump,
            in_flg_multis,
            in_flg_multisites,
            in_flg_residual,
            in_flg_assump_numpolyps,
            in_flg_dx_size,
            in_flg_dx_site,
            in_flg_dx_multis,
            in_specimen_type,
            in_notes,
            in_t_class,
            in_n_class,
            in_y_prefix,
            in_record_complete,
            in_flg_size_discrep
        );
    end if;

    select 'Record Updated' into lcl_message;

    return query select 
        in_specimen_id, lcl_message;
end;
$BODY$
language plpgsql

