...
begin
    ...
    if exists(select * from facility where facility_id = in_facility_id) then
        ...
        address3 = initcap(lower(in_address3)),
        ...
        contact1_name = initcap(lower(in_contact1_name)),
        contact1_phone = in_contact1_phone,
        contact1_email = in_contact1_email,
        contact1_type = initcap(lower(in_contact1_type)),
        contact2_name = initcap(lower(in_contact2_name)),
        contact2_phone = in_contact2_phone,
        contact2_email = in_contact2_email,
        contact2_type = initcap(lower(in_contact2_type)),
        contact3_name = initcap(lower(in_contact3_name)),
        contact3_phone = in_contact3_phone,
        contact3_email = in_contact3_email,
        contact3_type = initcap(lower(in_contact3_type)),
        ...
        initcap(lower(in_address3)),
        initcap(lower(in_city)),
        upper(in_state),
        in_zip,
        in_fax,
        in_status,
        ...
        in_ctc,
        in_comments,
        initcap(lower(in_contact1_name)),
        in_contact1_phone,
        in_contact1_email,
        initcap(lower(in_contact1_type)),
        initcap(lower(in_contact2_name)),
        in_contact2_phone,
        in_contact2_email,
        initcap(lower(in_contact2_type)),
        initcap(lower(in_contact3_name)),
        in_contact3_phone,
        in_contact3_email,
        initcap(lower(in_contact3_type)),
        in_facility_type
        );
    end if;
