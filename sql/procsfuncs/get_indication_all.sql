CREATE OR REPLACE FUNCTION get_indication_all(
...
begin
    ...
    return query select indication_calculated,count(*) as total  from Findings where completed = 1 and 
            endoscopist_id = lcl_endoscopist_id and facility_id = lcl_facility_id and exam_date >= in_start_dt and exam_date <= in_end_dt group by indication_calculated;
end;
$BODY$
LANGUAGE plpgsql 
